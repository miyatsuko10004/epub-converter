...
**Windowsの場合:**
* Calibreをインストールし、インストーラーのオプションで「Add to PATH」を選択するか、手動でシステムパスを通してください。

**macOSの場合:**
* Homebrewを使用する場合: `brew install --cask calibre`
* 手動インストールの場合: 公式サイトからインストールしてください。パス設定は不要です（スクリプトが自動検出します）。

### 2. UnRARユーティリティのインストール (RAR対応の場合)
`.rar`ファイルを取り扱う場合は、OSに**`unrar`**コマンドラインユーティリティをインストールする必要があります。

**Windowsの場合:**
* [WinRAR](https://www.rarlab.com/)などをインストールし、`unrar.exe`のディレクトリにパスを通してください。

**macOSの場合:**
* Homebrewを使用する場合: `brew install rar` または `brew install unrar`
